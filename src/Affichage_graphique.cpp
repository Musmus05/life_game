...
void Affichage_graphique::afficher_grille()
{
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event)) //fenetre principale
        {
            if (event.type == Event::Closed) //verification si fermé ou non
            {
                window.close();
            }
        }

        while (windowCompteurs.pollEvent(event))//verification si fermé ou non
        {
